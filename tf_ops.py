...
from third_party.batch_norm import BNLSTMCell
...
def bidir_rnn(x, token_embed, layers, seq_len=None, cell=tf.nn.rnn_cell.BasicLSTMCell):
    """
    ...
    return prediction_states, hidden_states, final_hidden


def bidir_lm_rnn(x, t, token_embed, layers,
                 seq_len=None, context_vector=None,
...
# ================ LOSS FUNCTIONS ============================
# ============================================================
def rnn_lm_loss(truth, h, dimension, scale_range=1.0,
                type='softmax', sampled_values=None):
    """
    ...
    :param type: What loss type to use. Can be 'softmax', 'sampled_softmax', or 'nce'
    ...
    if type == 'softmax':
        ...
        loss_column = tf.reshape(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                                labels=truth),
                                 [-1])  # [batch_size * sequence_length]
    elif type == 'nce':
        h = tf.stack(h)
        loss_column = tf.nn.nce_loss(tf.transpose(U), b, tf.reshape(truth, (-1, 1)),
                                     tf.reshape(tf.transpose(h, perm=(1, 0, 2)), (-1, fan_in)),
                                     sampled_values[0].get_shape().as_list()[0],
                                     dimension,
                                     num_true=1,
                                     ...
                                     name='nce_loss')
    elif type == 'sampled_softmax':
        h = tf.stack(h)
        loss_column = tf.nn.sampled_softmax_loss(tf.transpose(U), b, tf.reshape(truth, (-1, 1)),
                                                 tf.reshape(tf.transpose(h, perm=(1, 0, 2)), (-1, fan_in)),
                                                 sampled_values[0].get_shape().as_list()[0],
                                                 dimension,
                                                 num_true=1,
...
def batch_norm_rnn(inputs,
                   ...
                   state_index=-1):
    """
    :param inputs: (list of tf.Tensor) A list with length the number of time steps of longest sequence in the batch. inputs contains
                    tensors of shape=[num_sequences X feature_dimension]
    :param training: (tf.Placeholder) placeholder for whether or not to use batch statistics (during training) or population statistics (during evaluation)
    :param initial_state: (LSTM State Tuple) Initialized first hidden states. A  tuple of len(layers) tuples of cell and hidden state tensors
    :param layers: (list of int) list of number of nodes in each of stacked lstm layers
    :param sequence_lengths: (tf.Tensor, shape=[batch_size]) A vector of sequence lengths of size batch_size
    :param state_index: (int) If -1, last state is returned, if None all states are returned, if 1, second state is returned.
    :return: hidden_states, current_state
    """

    stack = [BNLSTMCell(num_units, training) for num_units in layers]
    cell = nn.rnn_cell.MultiRNNCell(stack, state_is_tuple=True)
